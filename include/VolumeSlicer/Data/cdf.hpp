  ...
  public:
    ...
    auto GetCDFValArray() -> std::vector<uint32_t>
    {
        std::sort(cdf.begin(), cdf.end(),
                  [this](CDFItem const &c1, CDFItem const &c2) { return GetFlatID(c1) < GetFlatID(c2); });
    ...
    void GenerateCDF()
    {
        ...
        if (all_empty)
        {
            ...
            {
                m[{it.x, it.y, it.z}] =
                    (std::min)({(std::min)(it.x + 1, dim_x - it.x), (std::min)(it.y + 1, dim_y - it.y),
        ...
        if (!all_empty)
        {
            auto IsBoard = [this](const std::array<int, 3> &idx) {
                if (idx[0] == 0 || idx[1] == 0 || idx[2] == 0 || idx[0] == dim_x - 1 || idx[1] == dim_y - 1 ||
                    idx[2] == dim_z - 1)
                {
                    return true;
                }
    ...
    CDF(int block_length, int len_x, int len_y, int len_z, SizeType iid = 0)
        : block_length(block_length), dim_x(RoundUp(len_x) >> static_cast<int>(log2(block_length))),
          dim_y(RoundUp(len_y) >> static_cast<int>(log2(block_length))),
          dim_z(RoundUp(len_z) >> static_cast<int>(log2(block_length)))
    {
...
inline void CDFGenerator::SetVolumeBlockData(CDFGenerator::VolumeBlock block, int volume_block_length,
...
class CDFManager
{
  ...
  public:
    ...
    int GetCDFBlockLength() const
    {
        return cdf_block_length;
    }

    int GetVolumeBlockLength() const
    {
        return volume_block_length;
    }
...
inline CDFManager::CDFManager() : volume_block_length(0), cdf_block_length(0), compute(false)
{
}

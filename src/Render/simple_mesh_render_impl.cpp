...
//
#include "Render/simple_mesh_render_impl.hpp"
...
SimpleMeshRendererImpl::SimpleMeshRendererImpl(int w, int h) : window_w(w), window_h(h)
{
    initGL();
    SimpleMeshRendererImpl::resize(w, h);
    this->simple_mesh_render_shader = std::make_unique<Shader>();
    this->simple_mesh_render_shader->setShader(shader::simple_mesh_render_v, shader::simple_mesh_render_f);
}
...
void SimpleMeshRendererImpl::SetMesh(std::shared_ptr<Mesh> mesh)
{
    ...
    for (int i = 0; i < surfaces.size(); i++)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, surfaces[i].vertices.size() * sizeof(Mesh::Vertex), surfaces[i].vertices.data(),
                     GL_STATIC_DRAW);
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, surfaces_ebo[i]);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, surfaces[i].indices.size() * sizeof(uint32_t), surfaces[i].indices.data(),
                     GL_STATIC_DRAW);
...
void SimpleMeshRendererImpl::deleteGLResource()
{
    setCurrentCtx();
    glDeleteVertexArrays(surfaces_vao.size(),surfaces_vao.data());
    glDeleteBuffers(surfaces_vbo.size(),surfaces_vbo.data());
    glDeleteBuffers(surfaces_ebo.size(),surfaces_ebo.data());
    GL_CHECK
}

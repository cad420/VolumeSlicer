...
#include "Render/cpu_raw_render_impl.hpp"
#include "Render/transfer_function_impl.hpp"
#include "Texture/texture_file.hpp"

VS_START

CPURawVolumeRendererImpl::CPURawVolumeRendererImpl(int w, int h) : window_w(w), window_h(h)
{
...
{
    this->volume_data_array = Linear3DArray<uint8_t>(raw_volume->GetVolumeDimX(), raw_volume->GetVolumeDimY(),
                                                     raw_volume->GetVolumeDimZ(), raw_volume->GetData());
    ...
    this->volume_data = TextureFile::LoadTexture3DFromMemory(raw_volume->GetData(), raw_volume->GetVolumeDimX(),
...
void CPURawVolumeRendererImpl::render(bool sync)
{
    Vec3d camera_pos = {camera.pos[0], camera.pos[1], camera.pos[2]};
    Vec3d view_front = Normalize(Vec3d{camera.look_at[0] - camera.pos[0], camera.look_at[1] - camera.pos[1], camera.look_at[2] - camera.pos[2]});
    ...
    auto GetEmptySkipPos = [this](Vec3d const &sample_pos, const Vec3d &sample_dir) {
        ...
        if (cdf_block_idx.x < 0 || cdf_block_idx.x >= cdf_dim_x || cdf_block_idx.y < 0 ||
    ...
    auto PhongShaing = [voxel, ka, ks, kd, shininess, this](Vec3d sample_pos, Vec3d diffuse_color,
                                                            Vec3d view_direction) -> Vec3d {
        Vec3d N;
        double x1, x2;
        ...
        for (int col = 0; col < window_w; col++)
        {
            ...
            for (int i = 0; i < 9000; i++)
            {
                ...
                if (sample_scalar > 0.0)
                {
                    ...
                    {
                        color +=
                            sample_color * Vec4d(sample_color.a, sample_color.a, sample_color.a, 1.0) * (1.0 - color.a);
                    }
                }
                if (color.a > 0.99)
                    break;
...
auto CPURawVolumeRendererImpl::GetBackendName() -> std::string
{
    return "cpu";
}

...
__device__ float3 PhongShading(int lod, int lod_t, stdgpu::unordered_set<int4, Hash_Int4> &missed_blocks,
                               float3 const &sample_pos, float3 const &diffuse_color, float3 const &view_direction);
...
__global__ void CUDAGenRays()
{
    ...
    float3 pixel_view_pos = cudaOffCompRenderParameter.camera_pos + cudaOffCompRenderParameter.front +
                            cudaOffCompRenderParameter.right * x + cudaOffCompRenderParameter.up * y;
    ...
    if (IsIntersected(intersect_t.x, intersect_t.y))
    {
        ...
        {
            ray_start_pos[image_idx] =
                make_float4(cudaOffCompRenderParameter.camera_pos + intersect_t.x * pixel_view_direction, 0);
        }
        else
        {
...
{
    if (sample_pos.x < 0 || sample_pos.y < 0 || sample_pos.z < 0 || sample_pos.x > compVolumeParameter.volume_dim.x ||
        sample_pos.y > compVolumeParameter.volume_dim.y || sample_pos.z > compVolumeParameter.volume_dim.z)
    {
        //        scalar = 0.f;
        return -1;
    }
    ...
    float3 offset_in_no_padding_block =
        (sample_pos - make_float3(virtual_block_idx * compVolumeParameter.no_padding_block_length * lod_t)) / lod_t;
    ...
    scalar = tex3D<float>(cache_volumes[physical_texture_idx], physical_sample_pos.x, physical_sample_pos.y,
...
__device__ float CalcDistanceFromCameraToBlockCenter(const float3 &sample_pos, int lod_t)
{
    ...
    float3 virtual_block_center =
        make_float3(virtual_block_idx + 0.5) * lod_t * compVolumeParameter.no_padding_block_length;
        ...
        if (sample_scalar > 0.f)
        {
            ...
            if (sample_color.w > 0.f)
            {
                ...
                auto shading_color = make_float4(PhongShading(cur_lod, cur_lod_t, missed_blocks,
                                                              ...
                                                              make_float3(sample_color), ray_direction),
                                                 sample_color.w);
                color = color + shading_color * make_float4(sample_color.w, sample_color.w, sample_color.w, 1.f) *
                                    (1.f - color.w);
                if (color.w > 0.99f)
                {
                    break;

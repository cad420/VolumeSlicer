...
class AABB
{
  public:
    ...
    void Union(const glm::vec3 &p)
    {
        ...
        max_p = {std::fmax(max_p.x, p.x), std::fmax(max_p.y, p.y), std::fmax(max_p.z, p.z)};
    }
    ...
    {
        return (index[0] == aabb.index[0] && index[1] == aabb.index[1] && index[2] == aabb.index[2] &&
                index[3] == aabb.index[3]);
    }
    ...
    float get_half_length() const
    {
        ...
        {
            LOG_ERROR("error! not cubic aabb!");
            return 0.f;
        }
    }
  ...
  public:
    OBB(glm::vec3 center_pos, glm::vec3 x, glm::vec3 y, glm::vec3 z, float half_x, float half_y, float half_z)
        : center_pos(center_pos), unit_x(glm::normalize(x)), unit_y(glm::normalize(y)), unit_z(glm::normalize(z)),
          half_wx(half_x), half_wy(half_y), half_wz(half_z)
    {
        updateHalfLenVec();
    }
...
{
    return OBB((min_p + max_p) / 2.f, {1.f, 0.f, 0.f}, {0.f, 1.f, 0.f}, {0.f, 0.f, 1.f}, (max_p.x - min_p.x) / 2.f,
               (max_p.y - min_p.y) / 2.f, (max_p.z - min_p.z) / 2.f);
}
...
class Pyramid
{
  public:
    Pyramid(glm::vec3 start_pos, glm::vec3 lu_pos, glm::vec3 ru_pos, glm::vec3 ld_pos, glm::vec3 rd_pos)
        : start_pos(start_pos), left_up_pos(lu_pos), right_up_pos(ru_pos), left_down_pos(ld_pos),
          right_down_pos(rd_pos), is_valid_pyramid(can_get_valid_obb())
    {
        calc_coefficient();
    }
    ...
    OBB getOBB() const
    {
        ...
        return OBB(obb_center_pos, obb_x, obb_y, obb_z, glm::length(obb_x) / 2, glm::length(obb_y) / 2,
                   glm::length(obb_z) / 2);
    }
...
inline void Pyramid::calc_coefficient()
{
    if (!is_valid_pyramid)
    {
        LOG_ERROR("calc invalid pyramid coefficient");
        return;
    }
...
inline bool Pyramid::intersect_aabb(const AABB &aabb)
{
    ...
    auto point_to_plane_dist=[](const glm::vec3& point,const std::array<float,4>& plane)->float{
        return
    (plane[0]*point.x+plane[1]*point.y+plane[2]*point.z+plane[3])/sqrt(plane[0]*plane[0]+plane[1]*plane[1]+plane[2]*plane[2]);
    };
    ...
    */
    ...
    if (!aabb.isCubic())
    {
       LOG_ERROR("intersect_aabb: input aabb is not cubic!");
    }
    auto get_box_visibility_against_plane = [](const AABB &aabb, const std::array<float, 4> &plane) -> BoxVisibility {
        ...
        glm::vec3 max_point = {(normal.x > 0.f) ? aabb.max_p.x : aabb.min_p.x,
        ...
        glm::vec3 min_point = {(normal.x > 0.f) ? aabb.min_p.x : aabb.max_p.x,

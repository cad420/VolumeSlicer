...
MeshImpl::MeshImpl(std::string path)
{
    ...
    this->mesh_loader =
        std::unique_ptr<IMeshLoaderPluginInterface>(PluginLoader::CreatePlugin<IMeshLoaderPluginInterface>(".obj"));
    if (!mesh_loader)
    {
    ...
    for (auto &shape : shapes)
    {
        ...
        {
            surface.vertices.emplace_back(Vertex{v.pos, v.normal});
        }
...
void MeshImpl::Transform(float space_x, float space_y, float space_z)
{
    ...
    {
        std::transform(surface.vertices.begin(), surface.vertices.end(), surface.vertices.begin(),

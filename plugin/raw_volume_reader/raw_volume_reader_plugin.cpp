...
template<>
static void LoadRawVolumeData<uint8_t>(std::ifstream& in,std::vector<uint8_t>& volume_data){
    if(!in.is_open()){
        throw std::runtime_error("LoadRawVolumeData pass ifstream not opened");
    }
    in.seekg(0,std::ios::end);
...
void RawVolumeReaderPlugin::Open(const std::string &filename, vs::VoxelType type,

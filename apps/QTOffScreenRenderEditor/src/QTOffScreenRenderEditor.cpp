...
void QTOffScreenRenderEditor::open(std::string filename){
    ...
    try{
        ...
        setting_widget->volumeLoaded(raw_volume_path, dim_x, dim_y, dim_z, space_x, space_y, space_z);
    }
...
void QTOffScreenRenderEditor::CreateWidgets()
{
    ...
    volume_render_widget->setWidget(setting_widget,offscreen_render_setting_widget);
}